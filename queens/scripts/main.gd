...
func _build_level_buttons() -> void:
	...
	for i in Levels.LEVELS.size():
		...
		if best_times.has(i):
			text += "\nBest %s" % _format_time(best_times[i])
		btn.text = text
...
func _start_level(index: int) -> void:
	current_level = index
	var lv: Dictionary = Levels.LEVELS[index]
	board.load_level(lv)
...
func _on_solved() -> void:
	...
	if not best_times.has(current_level) or elapsed < float(best_times[current_level]):
		best_times[current_level] = elapsed
		_save_progress()
	win_time_label.text = "Time: %s" % _format_time(elapsed)
	next_button.visible = current_level + 1 < Levels.LEVELS.size()
	win_overlay.visible = true


func _on_next() -> void:
	if current_level + 1 < Levels.LEVELS.size():
...
func _load_progress() -> void:
	...
	for key in cfg.get_section_keys("best_times"):
		best_times[int(key)] = cfg.get_value("best_times", key)


func _save_progress() -> void:
	var cfg := ConfigFile.new()
	for key in best_times:
		cfg.set_value("best_times", str(key), best_times[key])
	cfg.save(SAVE_PATH)
